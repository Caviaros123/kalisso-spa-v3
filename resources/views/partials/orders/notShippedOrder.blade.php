...
@forelse(\App\Order::where('shipped', '0')->get()->sortByDesc('created_at') as $order)


  <article class="card card-product  shadow-lg mb-4">
    <header class="card-header bg-warning text-light" >
      <div class="row-sm" >
       ...
       <div class="col-4 m-auto text-center">
       		@if($order->shipped == 0)
        		<h5><span class="badge  p-2 badge-light   shadow">Statut: En attente...</span></h5>
        ...
        <table class="table table-hover">
          <tbody>
           ...
           <tr>
            ...
            <?php   $getAdress = \App\Profile::where('email', $product->email)->first(); ?>

            <td>
               <div class="row">
               <strong>Vendeur</strong> <br> 
                 <div>  
                     {{ 'Ville: '.$getAdress->town}}  
                 </div>
                 <div>  
                    {{ 'Adresse: '.$getAdress->adress }} 
                 </div>
                 <div>  
                    {{$product->email }}
                 </div>
